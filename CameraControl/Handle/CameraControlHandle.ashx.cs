...
namespace CameraControl.Handle
{
    ...
    public class CameraControlHandle : IHttpHandler
    {
        ...
        public void SendCodeToCamera(HttpContext context)
        {
            ...
            if (!string.IsNullOrWhiteSpace(CodeType))
            {
                try
                {
                    ...
                    string result = Com_Data_Send(Constant.comType, code, datalength);
        ...
        public void SendCodeToCamera_Far(HttpContext context)
        {
            ...
            try
            {
                List<string> list = new List<string>() { Constant.jiaoju_add1, Constant.jiaoju_add2, Constant.jiaoju_add3 };
                string result = string.Empty;
                Timer timer = new Timer();
                timer.Interval = 700;
                timer.Elapsed += (object sender, ElapsedEventArgs e) =>
                {
                    result = Com_Data_Send(Constant.comType, list, datalength);
                };
                timer.Start();
        ...
        public void SendCodeToCamera_Near(HttpContext context)
        {
            ...
            try
            {
                List<string> list = new List<string>() { Constant.jiaoju_reduce1, Constant.jiaoju_reduce2, Constant.jiaoju_reduce3 };
                string result = string.Empty;
                Timer timer = new Timer();
                timer.Interval = 300;
                timer.Elapsed += (object sender, ElapsedEventArgs e) =>
